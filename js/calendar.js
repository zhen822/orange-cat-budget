 ...
 * Shows daily expenses, income totals, and meal thumbnails per day.
 */

import { getAllTransactions, getMealsByDate } from './storage.js';
...
async function openDayDetail(dateStr) {
  ...
  if (dayTx.length === 0 && meals.length === 0) {
  ...
  if (meals.length > 0) {
    html += `<h4 class="section-label">🍱 Meals</h4>
      <div class="meal-gallery">
        ${meals.map((m) => `
          <div class="meal-thumb">
            ${m.photoDataUrl ? `<img src="${m.photoDataUrl}" alt="${escHtml(m.description)}" class="meal-thumb__img">` : '<div class="meal-thumb__placeholder">🍽️</div>'}
