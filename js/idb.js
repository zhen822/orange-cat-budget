/**
 * idb.js — IndexedDB implementation (local fallback)
 * Used when no Firebase user is signed in.
 * Same function signatures as firebase.js.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (e) => {
      ...
      if (!database.objectStoreNames.contains(STORES.meals)) {
        const s = database.createObjectStore(STORES.meals, { keyPath: 'id', autoIncrement: true });
        s.createIndex('date', 'date', { unique: false });
      }
...
export async function addMeal(data) {
  const id = await p(getStore(STORES.meals, 'readwrite').add({ ...data, createdAt: Date.now() }));
  return { ...data, id };
}
export async function getMealsByDate(dateStr) {
  const all = await p(getStore(STORES.meals).getAll());
  return all.filter((m) => m.date === dateStr);
}
export async function getAllMeals() {
  return p(getStore(STORES.meals).getAll());
}
export async function deleteMeal(id) {
  await p(getStore(STORES.meals, 'readwrite').delete(id));
}
...
export async function exportAllData() {
  const [transactions, categories, recurring, meals, savingsGoals, settings] = await Promise.all([
    getAllTransactions(), getAllCategories(), getAllRecurring(),
    getAllMeals(), getAllSavingsGoals(), getAllSettings(),
  ]);
